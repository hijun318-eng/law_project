...
### 5-1. 1기: 단순 RAG의 한계 (Vocabulary Mismatch 발견)
...
### 5-2. 2기: 메타데이터 + 프롬프트 보완, LLM 최소화 원칙 도출

1기의 문제를 해결하기 위해 두 가지 접근을 시도했다:
...
### 5-3. 3기: BM25+Vector 하이브리드, Rerank 및 Query Rewrite 실험
...
Vocabulary Mismatch를 우회하기 위해 발상을 전환했다. 사용자 사례와 형식이 가장 유사한 문서는 **고용노동부 질의회시집**이라는 가정하에, 질의회시를 중간 매개체로 삼는 파이프라인을 실험했다.

```
사용자 질의 → 질의회시 Vector Search → 법령 Vector Search → 판례 Vector Search → LLM 답변
```

그러나 질의회시집 자체가 법률 용어로 구성된 경우가 많아 검색 품질이 기대만큼 개선되지 않았고, 올바른 질의회시를 가져와도 쟁점 추출 단계가 여전히 불안정했다.

질의회시 경로는 프로토타입 단계에서는 채택되지 않았으나, 데이터는 별도 ChromaDB 컬렉션(`admin_interpretations`)으로 구축하여 향후 파이프라인 확장 가능성을 열어두었다.

### 5-5. 5기: SAC 구조 도입 — 검색과 생성의 역할 분리 (핵심 돌파구)
...
### 5-6. 6기: 카테고리 기반 SAC 세분화

SAC 요약을 단일 형식으로 생성하는 대신, 판례의 **카테고리 메타데이터(임금, 해고, 산재, 직장 내 괴롭힘 등)에 따라 요약 방식을 차등 적용**했다. 같은 판례라도 임금 분쟁 맥락에서의 핵심 쟁점과 해고 분쟁 맥락에서의 핵심 쟁점은 다르기 때문이다. 이를 통해 검색 정밀도와 관련성이 한층 향상되었다.

### 5-7. LLM 설정 의사결정

법률 도메인의 특수성을 고려하여 `temperature=0`으로 고정 설정했다. 이는 동일 질의에 대해 항상 동일한 답변을 생성하여 법률 답변의 **재현성(Reproducibility)과 일관성(Consistency)**을 확보하기 위한 결정이다.

생성 파라미터를 고정하는 대신, 검색 단계의 다단계 Retrieval(판례 검색 → 리랭킹 → 참조조문 추출 → 법령 검색)을 통해 질의의 복잡도에 따라 LLM 입력 컨텍스트가 동적으로 구성되도록 설계했다. 즉, **생성 파라미터 변동 대신 Retrieval 기반의 동적 컨텍스트 제어** 방식을 채택한 것이다.

### 5-8. 진화 단계 요약

| 단계 | 변화 | 핵심 발견 | 영향 |
|:----:|------|-----------|:----:|
| **1기** | 단순 Vector RAG | Vocabulary Mismatch 문제 확인 | 근본적 재설계의 계기 |
| **2기** | 메타데이터 + 프롬프트 엔지니어링 | LLM 의존성의 연쇄 오류 위험 | **LLM 최소화 원칙 수립** |
| **3기** | BM25+Vector, Rerank, Query Rewrite | 추가 LLM 호출이 오류 전파 | 결정론적 검색 구조 확정 |
| **4기** | 질의회시 경로 실험 | 중간 매개체 접근의 한계 확인 | DB는 구축, 파이프라인은 유보 |
| **5기** | **SAC 구조 도입** | 검색/생성 역할 분리의 효과 | **가장 큰 성능 향상** |
| **6기** | 카테고리 기반 SAC 세분화 | 맥락별 최적 요약의 필요성 | 정밀도 추가 향상 |
| **7기** | Temperature=0 고정 | 법률 도메인 일관성 확보 | 생성 품질 안정화 |
