...
근로계약서 이미지를 업로드하면 OCR → 근로계약서 검증 → LLM 구조화 추출 → 규칙 기반 위반 검사의 4단계 파이프라인으로 분석한다(`backend/ocr_contract/pipeline.py:16-38`).

```
사용자 이미지 업로드 (JPG/PNG)
    ↓
[STEP 1] OCR 텍스트 추출 (PaddleOCR)
    ↓
[STEP 2] 근로계약서 여부 검증 (2단 게이트)
    ├── GATE 1: 키워드 휴리스틱 (비용 0, 빠름)
    └── GATE 2: LLM 분류기 (정밀, Fallback)
    ↓
[STEP 3] LLM 구조화 추출 (Zero-shot)
    ↓
[STEP 4] 규칙 엔진 (필수기재사항 + 법정기준 위반)
    ↓
결과 출력 (Streamlit UI)
```

### 4-2. OCR 엔진 — PaddleOCR

**도구**: PaddleOCR (`paddleocr` 라이브러리) — 한국어 사전 학습 모델 사용

| 구성 | 내용 |
|------|------|
| **OCR 엔진** | PaddleOCR (`lang="korean"`) |
| **이미지 전처리** | 최대 너비 1,500px로 리사이즈 (비율 유지, LANCZOS 보간) |
| **신뢰도 임계값** | 0.5 이상인 텍스트 박스만 수집 |
| **지연 초기화** | 첫 호출 시에만 모델 로딩 (`functools.lru_cache` 패턴) |
| **출력 형식** | 인식된 텍스트 박스들을 줄바꿈으로 결합한 단일 문자열 |

PaddlePaddle 3.x PIR OneDNN 호환성 문제를 우회하기 위해 `FLAGS_enable_pir_api=0` 환경변수를 설정한다(`ocr_engine.py:18`).

### 4-3. 2단 게이트 — 근로계약서 검증

업로드된 문서가 근로계약서인지 **2단 게이트**로 검증한다(`contract_gate.py`).

**GATE 1 — 키워드 휴리스틱**

| 검증 기준 | 상세 |
|-----------|------|
| 키워드 그룹 | (1) 계약 유형: "근로계약", "고용계약" / (2) 핵심 조항: "임금", "소정근로시간" / (3) 당사자: "사용자", "근로자" |
| 통과 조건 | 3개 그룹 중 2개 이상 충족 |
| 즉시 거절 | "영수증", "이력서", "진단서" 등 명백한 비계약서 키워드 감지 시 |
| 결과 | pass(✅) / reject(❌) / uncertain(→ GATE 2) |

**GATE 2 — LLM 분류기 (Zero-shot)**

GATE 1이 uncertain인 경우 LLM(gpt-5.4-nano)이 OCR 텍스트를 보고 근로계약서 여부를 판단한다. `true` / `false` / `uncertain` 세 가지로 응답하며, uncertain인 경우에도 통과 처리(보수적 설계)한다. LLM 오류 시에도 통과하여 서비스 연속성을 확보한다.

### 4-4. LLM 구조화 추출 (Zero-shot 프롬프트)

OCR로 추출된 텍스트에서 **8개 필수 계약 항목**을 JSON으로 구조화 추출한다(`llm/extractor.py`).

**프롬프트 전략**: **Zero-shot** (예제 없음, 규칙만 제시)

| 항목 | 설명 |
|------|------|
| **프롬프트 방식** | Zero-shot — Few-shot 예제를 제공하지 않고 추출 규칙만 명시 |
| **LLM 모델** | gpt-5.4-nano (temperature=0, 일관성 확보) |
| **입력** | OCR 텍스트 (최대 4,000자) |
| **출력** | JSON (8개 필드: 임금, 근무장소, 담당업무, 소정근로시간, 휴게시간, 휴일, 연차유급휴가, 계약기간) |

**프롬프트 설계 특징** (`constants.py:46-90`):

```
[역할 한정] 노무 전문가, 오직 "정보 추출"만 수행
[추출 규칙]
  - OCR 오타 보정 (근료→근로, 휴계→휴게, 제결→체결)
  - 양식만 있고 값이 비어있는 경우 null 표시
  - 추측 표현("추정", "것으로 보임") 사용 금지
  - 원문 그대로 추출 (계산·변환 금지)
[출력 형식] 순수 JSON (마크다운 없이)
```

LLM 출력에서 `json` 코드블록과 불필요한 마크다운을 제거한 후 `json.loads()`로 파싱한다. 파싱 실패 시 모든 필드를 null로 설정한 기본값을 반환하여 파이프라인이 중단되지 않도록 한다.

### 4-5. 규칙 엔진 — 법정기준 위반 검사

LLM이 추출한 필드값을 **결정론적 규칙 엔진(Deterministic Rule Engine)**으로 검사한다. LLM이 아닌 순수 코드 기반 검증이므로 환각(hallucination) 위험이 없다.

**필수기재사항 누락 검사** (`validators/required.py`)

근로기준법 제17조에 명시된 7개 필수기재사항이 모두 기재되었는지 확인한다. 공란(null), 빈 양식 패턴(`"년 월 일부터"`, `"원"` 단독 등)을 `blank_checker.py`의 정규식 패턴으로 탐지한다.

| 필수 항목 | 법적 근거 |
|-----------|-----------|
| 임금, 소정근로시간, 휴게시간, 휴일, 연차유급휴가, 근무장소, 담당업무 | 근로기준법 제17조, 제54조, 제55조, 제60조 |

**최저임금 위반 검사** (`validators/wage.py`)

| 항목 | 내용 |
|------|------|
| **검사 대상** | 시급 또는 월급에서 환산 시급 계산 |
| **기준** | 2025년 최저시급 **10,030원** (최저임금법 제6조) |
| **환산 기준** | 월급 → 시급: `월급 ÷ 209시간` (주 40시간 기준) |
| **파서** | `wage_parser.py` — 정규식 기반, "시급 N원" 또는 "월급 N원"/"기본급 N원" 패턴 인식 |

**근로시간·휴게시간 검사** (`validators/time.py`)

| 검사 항목 | 기준 | 법적 근거 |
|-----------|------|-----------|
| 1일 최대 근로시간 | 12시간 초과 시 위반 | 근로기준법 제50조·제53조 |
| 휴게시간 (4시간 이상 근로) | 최소 30분 | 근로기준법 제54조 제1항 |
| 휴게시간 (8시간 이상 근로) | 최소 60분 | 근로기준법 제54조 제1항 |

**파서 구성**

| 파서 | 입력 예시 | 추출값 |
|------|-----------|--------|
| `time_parser.py` | `09:00 ~ 18:00` | 9시간 |
| `break_parser.py` | `12:00 ~ 13:00`, `1시간` | 60분 |
| `wage_parser.py` | `시급 10,000원`, `월급 2,500,000원` | 10000, 2500000 |

### 4-6. Streamlit 프론트엔드

`frontend/pages/contract.py`에서 Streamlit UI를 제공한다.

| 기능 | 구현 |
|------|------|
| **파일 업로드** | `st.file_uploader` (JPG/PNG, 최대 제한 없음) |
| **이미지 미리보기** | 좌측 컬럼에 업로드된 이미지 표시 |
| **분석 실행** | "🔍 분석 시작" 버튼 → `analyze_contract()` 호출 |
| **결과 표시** | 3개 탭: (1) 계약 정보 (2) 필수기재사항 누락 (3) 법정기준 위반 |
| **예외 처리** | `NotAContractError` → "근로계약서가 아닙니다" 안내 + 상세 사유 표시 |

### 4-7. 독소 조항 분석에 관한 고찰

본 시스템은 "독소 조항(예: 초과근로 수당 미지급 조항, 부당한 위약금 조항)"을 **LLM이 직접 판단하지 않는다**. 대신 다음과 같은 계층적 접근을 취한다.

1. **LLM 구조화 추출**이 계약서에서 임금, 근로시간, 휴게시간 등 핵심 조건을 정확히 추출
2. **결정론적 규칙 엔진**이 추출된 값을 법정 기준(최저임금법, 근로기준법)과 비교하여 위반 여부를 판단
3. **수동 확인 필요 경고(warning)**는 금액 파싱이 불가능한 경우 등 모호한 상황에서 사용자에게 추가 확인을 요청

이 접근법의 장점은 LLM 환각(hallucination)에 의존하지 않고 명확한 법적 기준에 따라 위반을 판단할 수 있다는 점이다. 단, 계약서 조항의 문맥적 "독소성"(예: 복잡한 위약 조항, 부당한 입증 책임 전환 등)까지 분석하려면 향후 LLM 기반 계약서 리뷰 에이전트를 추가로 도입해야 한다.
